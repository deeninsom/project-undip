  ...
  const getDataMahasiswa = async () => {
    try {
      const respon = await axios.get(
        ` http://localhost:5000/api/v1/berkas/pkl?sort=${status}&limit=${limit}&page=${page}`,
              ...
              <nav className="navbar navbar-expand-lg navbar-light bg-transparent">
                <div className="container-fluid">
                  <div className="select-sortir col-md-2" >
                    <Form.Select aria-label="Default select example " onChange={sorting} >
                    ...
                    {dataPost.map((data) => {
                      return (
                        <tr key={data.id_pkl}>
                          ...
                          <td>{data.author.noInduk}</td>
                          <td>{data.author.angkatan}</td>
                          <td>{data.status_pkl}</td>
                          <td>{data.nilai.nilaiPkl}</td>
                        </tr>
                      );
                    })}
                  </tbody>
...
export default RekapProgresPkl;
