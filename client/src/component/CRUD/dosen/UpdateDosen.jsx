...
const UpdateDosen = ({ isUpdate, datas }) => {
  ...
  const getMahasiswaById = async () => {
    try {
      const response = await axios.get(
        ` http://localhost:5000/api/v1/dosen/${datas.id_dsn}`
      );
  ...
  const updateAdmin = async () => {
    ...
    try {
      await axios.put(` http://localhost:5000/api/v1/dosen/${datas.id_dsn}`, formData);
      reloadPage();
    } catch (error) {
      if (error.response) {
        setMsg(error.response.data.msg);
